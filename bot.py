...
from bot_tasks import toc, get_task, get_format_message, process_by_template, get_task_name
...
async def select_task(message: types.Message, state: FSMContext):
    try:
        ...
        m1 = await message.answer(f'Что-то пошло не так при выборе задания. Я получил на вход `{message.text.strip()}` и сломался.\nПопробуй заново.')
        ...
        m1 = await message.answer(f'Ты выбрал(а) задачу {get_task_name(task_index)}.\nОтправь входные данные в следующем формате:')
