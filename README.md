...
на выход получаем sympy выражение полинома $N-1$ степени.
...
Пусть функция $f(x)$ - Липшицева с константой $L$.
Метод ломанных использует эту константу, поэтому первым шагом либо её находим руками и передаём
как аргумент функции, либо ищем по нулевому шагу.

0. для поиска константы $L$ в случае дифференцируемой функции достаточно найти максимум
модуля производной. Для численного решения просто переберём целочисленные точки отрезка $[a,b]$
и найдём максимум.
1. Обозначим $g(x, y) = f(y) - L |x-y|$ - в этом шаге и далее.
Выбирается точка $x_0 \in [a, b]$. Строиться функция $p_0(x) = g(x, x_0) = f(x_0) - L|x-x_0|$ -
галочка с вершиной в точке $x_0$. Причём по построению $f(x) \leqslant p_0(x) \forall x \in [a, b]$. 
2. Следующая точка $x_1$ выбирается минимумом $p_0(x)$ на отрезке $[a, b]$, то есть получиться либо $a$, либо $b$ (в случае равенства выбираем любую).
3. Рассматривается функция $p_1(x) = \max\{ p_0(x), g(x, x_1) \}$ - ломанная, с экстремумами:
- максимумы в точках $x_0$ и $x_1$;
- минимумы в некоторых точках.
4. Каждый последующий шаг:
- $x_i=\text{argmin} p_{i-1}(x)$ - если несколько минимумов с одинаковыми значениями, выбирается любой $x$;
- $p_i(x) = \max\{ p_{i-1}(x), g(x, x_i) \}$.

См. функцию `method_lomannih` в [extremum.py](src/extremum.py). 

Usage:
```python
from src.extremum import method_lomannih
x = sp.Symbol('x')
f = x**3 + 6*x**2 + 9*x + 1
a = -3
b = 0
minimum = method_lomannih(f, a, b, x0 = -2, x=x)
print(minimum)
```

TODO:
- Вывод получившейся функции не очень
- Вероятно, лучше ещё генерировать графики, как мы это делали на семинарах.
